...
public class TryTransferTest {
    public static void main(String[] args) throws Exception {
        int producerSize = Runtime.getRuntime().availableProcessors(), consumerSize = 1000;
    ...
    private static long testQueue(String queueName, Queue<TransferPacket> queue, int producerSize, int consumerSize) throws Exception {
        ...
        FastTransferQueueConsumer[] fstConsumers = null;
        ...
        if (queue instanceof BlockingQueue) {//Blocking Queue Consumers
            ...
            fstConsumers = new FastTransferQueueConsumer[consumerSize];
            for (int i = 0; i < consumerSize; i++) {
                fstConsumers[i] = new FastTransferQueueConsumer(fastTransferQueue, pollStartCountLatch, pollEndCountLatch);
        ...
        if (queue instanceof LinkedTransferQueue) {//Blocking Queue Consumers
            ...
            for (int i = 0; i < producerSize; i++) {
                new FastTransferQueueTryTransferProducer((FastTransferQueue) queue, existConsumerInd, producersDownLatch).start();
            }
        }
   ...
   static final class TransferPacket {
    ...
    //FastTransferQueue
    static final class FastTransferQueueConsumer extends Consumer {
        private FastTransferQueue<TransferPacket> queue;
        public FastTransferQueueConsumer(FastTransferQueue<TransferPacket> queue, CountDownLatch pollStartLatch, CountDownLatch pollEndDownLatch) {
    ...
    static final class FastTransferQueueTryTransferProducer extends Producer {
        private FastTransferQueue<TransferPacket> queue;
        public FastTransferQueueTryTransferProducer(FastTransferQueue<TransferPacket> queue, AtomicBoolean activeInd, CountDownLatch producersDownLatch) {
