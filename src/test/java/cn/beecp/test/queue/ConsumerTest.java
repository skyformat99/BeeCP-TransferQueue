...
public class ConsumerTest {
    ...
    public static void main(String[] args) throws Exception {
        int producerSize =Runtime.getRuntime().availableProcessors(), consumerSize = 10, operateSize = 1000;
		...
		timeList.add(testQueue("FastTransferQueue", new FastTransferQueue<Object>(), producerSize, consumerSize, operateSize));
    ...
    private static long testQueue(String queueName, Queue<Object> queue, int producerSize, int consumerSize, int operateSize) throws Exception {
		...
		if(queue instanceof BlockingQueue){//Blocking Queue Consumers
			...
			FastTransferQueue<Object> fastTransferQueue=(FastTransferQueue<Object>)queue;
			fstConsumers = new FstQConsumer[consumerSize];
			for (int i = 0; i < consumerSize; i++) {
				fstConsumers[i] = new FstQConsumer(startTime,fastTransferQueue,consumersDownLatch,operateSize);
	...
	static final class FstQConsumer extends Thread {
		...
		private int operateSize;
		private CountDownLatch latch;
		private FastTransferQueue<Object> queue;
		public FstQConsumer(long startTime,FastTransferQueue<Object> queue, CountDownLatch latch,int operateSize) {
			this.queue = queue;
			this.latch = latch;
			this.startTime=startTime;
			this.operateSize=operateSize;
