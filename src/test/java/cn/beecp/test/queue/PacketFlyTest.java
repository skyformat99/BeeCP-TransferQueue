...
public class PacketFlyTest {
	...
	private static long testQueue(String queueName, Queue<TransferPacket>queue, int producerSize, int consumerSize) throws Exception {
		...
		if(queue instanceof BlockingQueue) {//Blocking Queue Consumers
			...
			FastTransferQueue<TransferPacket> fastTransferQueue=(FastTransferQueue<TransferPacket>)queue;
			for (int i = 0; i < producerSize; i++) {
				new FastTransferQueueOfferProducer(fastTransferQueue,existConsumerInd, producersDownLatch).start();
			}
		}
	...
	static final class FastTransferQueueOfferProducer extends Producer {
		private FastTransferQueue<TransferPacket> queue;
		public FastTransferQueueOfferProducer(FastTransferQueue<TransferPacket> queue, AtomicBoolean activeInd, CountDownLatch producersDownLatch) {
